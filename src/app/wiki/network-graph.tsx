...
interface WikiResponse {
  query: {
    ...
    pages: { [pageId: string]: { pageid: number; ns: number; title: string; thumbnail?: { source: string; width: number; height: number }; pageimage?: string } };
  };
}
...
const query = 'https://en.wikipedia.org/w/api.php?action=query&prop=pageimages&pithumbsize=600&format=json&origin=*&titles='

async function getImageURL(title: string) {
  try {
    const response = await fetch(query + convertToSnakeCase(title), {method: "GET"});
...
async function convertMapToGraph(mapData : MapItem[], startPage : string, endPage : string) {
  ...
  const graph: {
    nodes: { id: number; label: string; title: string, shape: string, image:string ,size: number, font: string, level: number}[];
...
const NetworkGraph: React.FC<Props> = ({ mapData, startPage, targetPage, showGraph }) => {
